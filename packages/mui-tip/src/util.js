...
export function computePlacementInfo(
  ref,
  container,
  target,
  limitQueue,
  ...
  const placementQueue =
    limitQueue && limitQueue.length ? limitQueue : DEFAULT_PLACEMENT_QUEUE;
  ...
  const processedQueue = Object.keys(margin).map(key => {
    ...
    const verFullBiasCheck =
      verSingleBiasCheck && margin.left.size > dw && margin.right.size > dw;
    // 左右方向上可容纳目标元素
    ...
    const horFullBiasCheck =
      horSingleBiasCheck && margin.top.size > dh && margin.bottom.size > dh;
